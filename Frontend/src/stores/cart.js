    ...
    actions: {
        async loadCart() {
            try {
                ...
                const response = await axios.get(`http://backend:8000/api/customers/customer-cart/${user_id}/`); //ดึง cart ที่ตรงกับ customer.id
        ...
        async addToCart(productData) {
            ...
            if (itemIndex >= 0) {
                ...
                try {
                    const response = await axios.post('http://backend:8000/api/customers/customer-cart/', productData);
        ...
        async updateQuantity(index, amount) {
            if (this.items[index]) {
                try {
                    ...
                    await axios.put(`http://backend:8000/api/customers/cart/${productId}/`, {
                        amount: amount,
                    });
        ...
        async removeItemInCart(index) {
            if (this.items[index]) {
                try {
                    ...
                    await axios.delete(`http://backend:8000/api/customers/cart/${productCartId}/`);
