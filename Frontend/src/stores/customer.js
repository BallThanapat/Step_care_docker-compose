...
export const useCustomerStore = defineStore('customer', {
    ...
    actions: {
        ...
        async addCustomer(customerData) {
            try {
                const response = await axios.post('http://backend:8000/api/customers/register/', customerData);
        ...
        async deleteAddress(addressId) {
            try {
                const response = await axios.delete(`http://backend:8000/api/customers/customer-addresses/${addressId}/`);
        ...
        async getAddress(id) {
            try {
                const response = await axios.get(`http://backend:8000/api/customers/customer-addresses/${id}/`);
                this.list = response.data;
                return this.list;
            } catch (error) {
        ...
        async createAddress(newAddress) {
            try {
                const response = await axios.post('http://backend:8000/api/customers/customer-addresses/', newAddress, {
                });
                this.list.push(response.data);
                this.loaded = true;
            } catch (error) {
        ...
        async updateAddress(addressId, updatedAddress) {
            try {
                const response = await axios.put(`http://backend:8000/api/customers/customer-addresses/${addressId}/`, updatedAddress, {
                });
        ...
        async createOrder(orderData) {
            try {
                const response = await axios.post('http://backend:8000/api/customers/orders/', 
        ...
        async getOrder() {
            ...
            try {
                const response = await axios.get(`http://backend:8000/api/customers/orders/${customerID}/`, 
                );
        ...
        async loadOrder() {
            try {
                const response = await axios.get(`http://backend:8000/api/customers/orders/`, 
                );
                console.log('Order data retrieved successfully:', response.data);
                this.orders = response.data;
                return response.data;
        ...
        async updateStatus(statusChange) {
            try {
                ...
                const response = await axios.put(`http://backend:8000/api/customers/edit_orders/`, statusChange, {
                    headers: {
